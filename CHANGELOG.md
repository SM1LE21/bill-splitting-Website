...
- **New `/pricing` page**, patterned on `/export-expenses-excel`: `metadata`, a `faqs` array with `FAQPage` JSON-LD as the first child of `<Layout>`, house styling. Deliberately quiet — informative, one badge, no urgency. Two plans (€24.99/year with "Save 30%", €2.99/month), **VAT-inclusive**, CTAs to `app.expensemate.app/upgrade?plan=…&ref=site-pricing`, a note that Premium bought on the web works in the iOS app on the same account, the free-vs-Premium table, what the free plan includes, and five FAQs (is Premium required, does web Premium work on iOS, how to cancel, what "unlimited" means, and **LINK.COM\*** on the statement). Listed in `sitemap.ts`; **footer only, no navbar entry** — owner decision.
- **`PREMIUM_ROWS` and `upgradeUrl()` moved to `src/constants/premiumPlans.ts`** so the homepage Premium section and `/pricing` cannot drift apart on what Free and Premium include. The Premium cards gained upgrade CTAs (`ref=site-premium`), and the footnote that called Premium "an in-app purchase, billed through the App Store" now names both sellers.
- **The comparison table overflowed at 360px** — `grid-cols-[1.4fr_1fr_1fr]` needs about 310px of content box once the "Multi-currency conversion" and "Unlimited" cells hit their min-content floors, which is more than a 360px phone leaves inside `px-6`. Both copies now scroll inside `overflow-x-auto` with a `22rem` floor so the header background still paints.
- **Six now-false "Premium is iOS-only" claims reworded**, three of them on `/vs-splitwise` (the cited line 46 had drifted; the real ones are the limitations list, the pricing FAQ and the **Android FAQ, which feeds the `FAQPage` JSON-LD**), plus `/export-expenses-excel`, `/downloads` and the homepage Android FAQ. Also the **`Roadmap` Android card**, which was not on the list but renders the same claim on the homepage. The `AggregateOffer` gained `url: https://app.expensemate.app/upgrade`. **Deliberately left:** the `ReleaseNotes` Web 1.0/1.1 entries, which are dated records of what those releases shipped, and the completed Web v1.1 roadmap bullet.
- **One bug fixed on the way:** the footer prefixed every product link with `/` off the homepage, which is right for `#features` and would have turned `/pricing` into the protocol-relative `//pricing`. `npm run lint` clean, `npm run build` clean, **19 routes**. Emitted HTML verified: four upgrade links with the right `ref` per surface, the offer `url`, the `FAQPage` block, `/pricing` in `sitemap.xml`, `href="/pricing"` (never `//pricing`) off the homepage.
